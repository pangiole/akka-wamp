...
Welcome(1,Map(roles -> Map(broker -> Map(), dealer -> Map()), agent -> akka-wamp-0.15.2))
Welcome(3,Map(roles -> Map(broker -> Map(), dealer -> Map()), agent -> akka-wamp-0.15.2))
...
/**
  ...
  */
class DefaultRouterSpec extends DefaultSpec {
  ...
  "The default router" should "auto-create realms on incoming HELLO('myrealm') and reply GOODBYE" in { f =>
    ...
    expectMsg(Welcome(1, Dict().withRoles("broker", "dealer").withAgent("akka-wamp-0.15.2")))
