...
import akka.wamp.Endpoint
import akka.wamp.messages.{Bind, Bound, CommandFailed, HandleHttpConnection, Unbind}
...
class ConnectionListener(binder: ActorRef, router: ActorRef, endpoint: String, sslContext: SSLContext)
  ...
  val (_, address, format) = Endpoint.fromConfig(config, endpoint).toTuple
