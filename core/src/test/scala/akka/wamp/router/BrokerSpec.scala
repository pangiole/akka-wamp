...
class BrokerSpec extends RouterBaseSpec {
  ...
  it should "reply ERROR on UNSUBSCRIBE unknown subscription" in { f =>
    ...
    client2.expectMsg(Error(Unsubscribe.tpe, 1, Error.defaultDict, "wamp.error.no_such_subscription"))
    
    client2.send(f.router, Unsubscribe(2, subscriptionId = 999))
    client2.expectMsg(Error(Unsubscribe.tpe, 2, Error.defaultDict, "wamp.error.no_such_subscription"))
  }
  ...
  it should "dispatch EVENT on PUBLISH to a topic with subscribers" in { f =>
    val client1 = TestProbe("client1")
    client1.send(f.router , Hello(details = Dict().withRoles("subscriber")))
    client1.receiveOne(0.seconds)
    
    val client2 = TestProbe("client2")
    client2.send(f.router , Hello(details = Dict().withRoles("subscriber", "publisher")))
    client2.receiveOne(0.seconds)
    
    val client3 = TestProbe("client3")
    client3.send(f.router , Hello(details = Dict().withRoles( "publisher")))
