...
class CustomConnectionHandlerSpec extends ConnectionHandlerBaseSpec {
  ...
  "A router.ConnectionHandler configured with custom settings" should "drop offending messages and resume" in { f =>
    ...
    // --> bad HELLO : myapp.realm
    f.client.sendMessage("""[1,"myapp.realm",{"roles":{"publisher":{}}}]""")
    // <-- ABORT
    f.client.expectMessage("""[3,{"message":"The realm 'myapp.realm' does not exist."},"wamp.error.no_such_realm"]""")

    // --> HELLO
    f.client.sendMessage("""[1,"default.realm",{"roles":{"publisher":{}}}]""")
    // <-- WELCOME
    f.client.expectMessage("""[2,1,{"agent":"akka-wamp-0.11.0","roles":{"broker":{},"dealer":{}}}]""")
    ...
    // --> HELLO
    f.client.sendMessage("""[1,"default.realm",{"roles":{"subscriber":{},"publisher":{},"callee":{}}}]""")
    // <-- WELCOME
    f.client.expectMessage("""[2,2,{"agent":"akka-wamp-0.11.0","roles":{"broker":{},"dealer":{}}}]""")
