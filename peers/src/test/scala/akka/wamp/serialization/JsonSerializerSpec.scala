...
class JsonSerializerSpec extends SerializingBaseSpec 
{
  ...
  it should "serialize Welcome object messages" in {
    val message = Welcome(1233242, Dict().setAgent("akka-wamp-0.11.0").addRoles(Roles.broker))
    whenReduced(s.serialize(message)) { json =>
      json mustBe """[2,1233242,{"agent":"akka-wamp-0.11.0","roles":{"broker":{}}}]"""
    }
  }
  ...
  it should "serialize Error object messages" in {
    val message1 = Error(Subscribe.tpe, 12345, Error.defaultDetails, "wamp.error.no_such_subscription")
    whenReduced(s.serialize(message1)) { json =>
      json mustBe """[8,32,12345,{},"wamp.error.no_such_subscription"]"""
    }
    val message2 = Error(Publish.tpe, 67890, Error.defaultDetails, "wamp.error.no_such_topic", Payload(args))
    whenReduced(s.serialize(message2)) { json =>
      json mustBe s"""[8,16,67890,{},"wamp.error.no_such_topic",$argsJson]"""
    }
    val message3 = Error(Call.tpe, 67890, Error.defaultDetails, "wamp.error.no_such_procedure", Payload(kwargs))
    whenReduced(s.serialize(message3)) { json =>
      json mustBe s"""[8,48,67890,{},"wamp.error.no_such_procedure",[],$kwargsJson]"""
    }
    val message4 = Error(Invocation.tpe, 45678, Error.defaultDetails, "wamp.error.no_such_procedure", Payload(args, kwargs))
