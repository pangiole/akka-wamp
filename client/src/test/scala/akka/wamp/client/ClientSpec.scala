...
class ClientSpec extends DefaultSpec {

  "A client" should "fail to connect when 'unknown' endpoint is given" in { f =>
    val conn = f.client.connect(endpoint = "unknown")
    whenReady(conn.failed) { ex =>
      ex mustBe a[ClientException]
      ex.getCause mustBe a[ConfigException.Missing]
      ex.getMessage mustBe "No configuration setting found for key 'endpoint.unknown'"
    }
  }
