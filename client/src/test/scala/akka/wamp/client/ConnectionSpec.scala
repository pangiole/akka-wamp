...
class ConnectionSpec extends DefaultSpec(ConfigFactory.parseString(
  """
    | akka {
    |   wamp {
    |     router {
  ...
  """.stripMargin))
{
  ...
  it should "fail to open session if it turns out to have been disconnected from router side" in { f =>
    pending
    f.withConnection { conn =>
      // TODO simulate disconnection from router side (maybe violating protocol)
